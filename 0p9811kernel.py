...
def DO(frm, to, fileno):
    ...
    train_df = pd.read_csv(path + "train.csv", parse_dates=['click_time'], skiprows=range(1, frm), nrows=to - frm,
                           dtype=dtypes,
                           usecols=['ip', 'app', 'device', 'os', 'channel', 'click_time', 'is_attributed'])

    print('loading test data...')
    if debug:
        test_df = pd.read_csv(path + "test_supplement.csv", nrows=100000, parse_dates=['click_time'], dtype=dtypes,
                              usecols=['ip', 'app', 'device', 'os', 'channel', 'click_time', 'click_id'])
    else:
        test_df = pd.read_csv(path + "test_supplement.csv", parse_dates=['click_time'], dtype=dtypes,
                              usecols=['ip', 'app', 'device', 'os', 'channel', 'click_time', 'click_id'])
    ...
    gc.collect()
    train_df = do_mean(train_df, ['ip', 'device', 'os'], 'app');
    gc.collect()
    ...
    train_df = do_mean(train_df, ['os', 'device', 'app', 'channel'], 'hour');
    gc.collect()
    train_df = do_countuniq(train_df, ['os', 'device', 'app', 'channel'], 'hour');
    gc.collect()
    ...
    print('loading test data...')
    if debug:
        dfTest = pd.read_csv(path + "test.csv", nrows=100000, parse_dates=['click_time'], dtype=dtypes,
                             usecols=['ip', 'app', 'device', 'os', 'channel', 'click_time', 'click_id'])
    else:
        dfTest = pd.read_csv(path + "test.csv", parse_dates=['click_time'], dtype=dtypes,
                             usecols=['ip', 'app', 'device', 'os', 'channel', 'click_time', 'click_id'])
    ...
    ip_dfTest_supplement = ip_dfTest_supplement.groupby(['ip',
                                                         ...
                                                         'os',
                                                         'channel',
                                                         'click_time'])['is_attributed'].agg(
        ['mean']).reset_index().rename(index=str,
                                       columns=({'ip': 'ip',
                                                 'mean': 'is_attributed'}))
